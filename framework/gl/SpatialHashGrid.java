...
public class SpatialHashGrid {
    ...
    public int[] getCellIds(GameObject obj) {
        ...
        int x2 = (int) Math.floor((obj.bounds.lowerLeft.x + obj.bounds.widht) / cellSize);
        int y2 = (int) Math.floor((obj.bounds.lowerLeft.y + obj.bounds.height) / cellSize);

        if (x1 == x2 && y1 == y2) { // in one cell
            ...
            cellIds[2] = -1;
            cellIds[3] = -1;
        } else if (x1 == x2) { // in two cells by row
            int i = 0;
        ...
        } else if (y1 == y2) { // in two cells by column
            int i = 0;
            if (y1 >= 0 && y1 < cellsPerCol) {
                if (x1 >= 0 && x1 < cellsPerRow)
                    cellIds[i++] = x1 + y1 * cellsPerRow;
            ...
            if (x1 >= 0 && x1 < cellsPerRow && y1 >= 0 && y1 < cellsPerCol) // left bottom
                cellIds[i++] = x1 + y1CellsPerRow;
            if (x2 >= 0 && x2 < cellsPerRow && y1 >= 0 && y1 < cellsPerCol) // right bottom
                cellIds[i++] = x2 + y1CellsPerRow;
            if (x2 >= 0 && x2 < cellsPerRow && y2 >= 0 && y2 < cellsPerCol) // right top
                cellIds[i++] = x2 + y2CellsPerRow;
            if (x1 >= 0 && x1 < cellsPerRow && y2 >= 0 && y2 < cellsPerCol) // left top
